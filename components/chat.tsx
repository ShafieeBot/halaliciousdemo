...
export default function ChatInterface({ places, onFilterChange, onSelectPlace }: ChatInterfaceProps) {
    ...
    const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string, places?: { name: string, cuisine: string }[] }[]>([]);
    ...
    const processMessage = async (messageMessages: typeof messages) => {
        try {
            ...
            if (response.status === 429) {
                ...
                setMessages(prev => [...prev, { role: 'assistant', content: `Server overloaded. Retrying in ${waitSeconds}s...` }]);
                // Wait
            ...
            try {
                ...
                const aiMessage = parsed.message || "Okay, I've updated the map.";
                ...
                setMessages(prev => [...prev, { role: 'assistant', content: "I couldn't understand that. Try asking differently." }]);
            }
